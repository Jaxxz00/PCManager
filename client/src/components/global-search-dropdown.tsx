...
import { Search, Monitor, User, X, Wrench, FileText } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { useLocation } from "wouter";
import { format } from "date-fns";
import { it } from "date-fns/locale";
import type { Asset, Employee, Document } from "@shared/schema";
...
export const GlobalSearchDropdown = memo(function GlobalSearchDropdown({ isOpen, onClose, searchTerm, onSearchChange }: GlobalSearchDropdownProps) {
  ...
  const { data: documents = [], isLoading: documentsLoading, isError: documentsError } = useQuery<Document[]>({
  ...
  // Filtra Documenti - ottimizzato con useMemo
  ...
  return (
    <div ref={dropdownRef} className="relative">
      ...
      <div className="absolute top-0 left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-xl z-50 max-h-96 overflow-y-auto min-w-full">
        <div className="p-4">
          {(assetsLoading || employeesLoading || maintenanceLoading || documentsLoading) && searchTerm.trim() ? (
            <div className="text-center py-6 text-muted-foreground">
              <div className="animate-spin h-6 w-6 border-2 border-primary border-t-transparent rounded-full mx-auto mb-3"></div>
              <p>Caricamento...</p>
            </div>
          ) : (assetsError || employeesError || maintenanceError || documentsError) && searchTerm.trim() ? (
            ...
            <>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            {document.description && (
                              <p className="text-xs text-blue-600 truncate">{document.description}</p>
                            )}
                            {document.tags && (
                              <div className="flex gap-1 mt-1 flex-wrap">
                                {document.tags.split(',').slice(0, 2).map((tag, index) => (
                                  <span key={index} className="text-xs bg-gray-50 text-gray-700 px-2 py-1 rounded whitespace-nowrap">
                                    {tag.trim()}
                                  </span>
                                ))}
                              </div>
                    ...
                    {filteredDocuments.length > 3 && (
                      <p className="text-xs text-muted-foreground text-center py-2 bg-gray-50 rounded">
                        +{filteredDocuments.length - 3} altri documenti... (vai ai documenti)
                      </p>
                    )}
                  </div>
                </div>
              ...
              {/* Nessun risultato */}
              {filteredAssets.length === 0 && filteredEmployees.length === 0 && filteredMaintenance.length === 0 && filteredDocuments.length === 0 && (
                <div className="text-center py-6 text-muted-foreground">
                  <Search className="h-8 w-8 mx-auto mb-3 opacity-50" />
                  <p>Nessun risultato per "{searchTerm}"</p>
                  <p className="text-sm mt-1">Prova con "Dell", "Luca", "IT", "smartphone", "RIC-" o "manleva"...</p>
                </div>
              )}
            </>
          )}
